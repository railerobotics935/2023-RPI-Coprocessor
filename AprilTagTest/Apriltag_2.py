...
def main():
    ...
    while True:
        ...
        for tag in filter_tags:
            ...
            cv2.line(output_img, corner0, corner1, color = col_box, thickness = 1)
            cv2.line(output_img, corner1, corner2, color = col_box, thickness = 1)
            cv2.line(output_img, corner2, corner3, color = col_box, thickness = 1)
            cv2.line(output_img, corner3, corner0, color = col_box, thickness = 1)

            # Label the tag with the ID:
            cv2.putText(output_img, f"{tag_id}", (int(center.x), int(center.y)), cv2.FONT_HERSHEY_SIMPLEX, 1, col_txt, 1)
